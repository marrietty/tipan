    ...
    <div class="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 pb-8 pt-4">
        <div class="grid grid-cols-1 lg:grid-cols-12 gap-8">
            ...
            <div class="lg:col-span-4 flex flex-col gap-6">
                <!-- Secondary Actions -->
                <a href="{{ route('patient.appointments.index') }}" class="group flex items-center justify-between rounded-xl px-4 py-4 text-body bg-white border border-line shadow-sm transition-all duration-300 hover:-translate-y-0.5 hover:shadow hover:border-primary/30">
                    <div class="flex items-start gap-3">
                        ...
                        <div>
                            ...
                            <div class="mt-0.5 text-xs text-body leading-relaxed">
                                @if($appointmentsCount > 0)
                                    {{ $appointmentsCount }} total {{ Str::plural('appointment', $appointmentsCount) }}
                                @else
                                    No upcoming visits yet.<br><span class="text-primary font-medium group-hover:underline">Book your first appointment &rarr;</span>
                                @endif
                            </div>
                        </div>
                    </div>
                    <svg class="h-4 w-4 text-gray-300 transition-colors group-hover:text-primary shrink-0" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M9 5l7 7-7 7"/></svg>
                </a>
                
                <a href="{{ route('patient.records.index') }}" class="group flex items-center justify-between rounded-xl px-4 py-4 text-body bg-white border border-line shadow-sm transition-all duration-300 hover:-translate-y-0.5 hover:shadow hover:border-primary/30">
                    <div class="flex items-start gap-3">
                        <div class="mt-0.5 text-primary shrink-0">
                        ...
                        <div>
                            ...
                            <div class="mt-0.5 text-xs text-body leading-relaxed">
                                ...
                                @else
                                    No records yet.<br><span class="text-primary font-medium group-hover:underline">Book an appointment &rarr;</span>
                                @endif
                            </div>
                        </div>
                    </div>
